...
<svelte-autofixer>
## 🔥 CRITICAL: Always Use svelte-autofixer

**MANDATORY for all Svelte code:**
```
Write code → svelte-autofixer → Fix issues → svelte-autofixer → Repeat until clean
```

**Never show code to user without autofixer approval.**

Parameters:
- `code`: Your Svelte code
- `desired_svelte_version`: 5 (default) or 4
- `filename`: Component name only (e.g., `App.svelte`, not full path)

</svelte-autofixer>

<runes>
## Svelte 5 Runes: What to Use When

### $state - Reactive Variables
Use for: Any variable that changes and needs to trigger UI updates

```svelte
<script>
  let count = $state(0);           // Simple value
  let user = $state({ name: '' }); // Object
  let items = $state([]);          // Array
</script>
```

**Common mistake:** Using plain `let` for reactive data
**Fix:** Always use `$state()` for data that changes

---

### $derived - Computed Values
Use for: Values calculated from other reactive state
...
**Common mistake:** Using `$state` for computed values
**Fix:** Use `$derived` - it's readonly and auto-updates

---

### $effect - Side Effects
Use for: Running code when state changes (logging, DOM manipulation, API calls)
...
<script>
  ...
  $effect(() => {
    ...
    // Auto re-runs when count changes
  });
</script>
```

**Common mistake:** Trying to return values from $effect
**Fix:** $effect is for side effects only, use $derived for computed values

---

### $props - Component Props
Use for: Accepting data from parent components

```svelte
<script>
  let { title, count = 0 } = $props();  // count has default value
</script>
...
**Common mistake:** Using `export let` (Svelte 4 syntax)
**Fix:** Use `$props()` destructuring

---

### $bindable - Two-Way Binding
Use for: Props that parent can bind to (e.g., form inputs)
...
**Common mistake:** Expecting props to be automatically bindable
**Fix:** Explicitly mark with `$bindable()`

</runes>

<quick-reference>
## Quick Rune Decision Tree

- Need a **variable that changes**? → `$state()`
- Need a **calculated value**? → `$derived()`
- Need to **do something when state changes**? → `$effect()`
- Need to **accept data from parent**? → `$props()`
- Need **parent to bind to this prop**? → `$bindable()`
...
**Common mistake:** Using `on:click` (Svelte 4 syntax)
**Fix:** Use `onclick` (no colon)
...
### list-sections + get-documentation
Always search docs before asking questions:
```
1. Call list-sections
2. Find relevant sections by use_cases
...
1. ✅ **ALWAYS use svelte-autofixer** before showing code
2. ✅ Use `$state()` for reactive variables, not plain `let`
3. ✅ Use `$derived()` for computed values, not `$state()`
4. ✅ Use lowercase event handlers: `onclick`, not `on:click`
5. ✅ Use `$props()` for component props, not `export let`
6. ✅ Keep running autofixer until it returns zero issues
